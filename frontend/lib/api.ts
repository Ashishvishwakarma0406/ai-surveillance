...
// Video Upload — goes directly to backend (bypasses Next.js proxy to avoid body size limits)
export const uploadVideo = async (file: File, onProgress?: (progress: number) => void) => {
    const formData = new FormData();
    formData.append('file', file);

    // Do not set Content-Type: multipart/form-data manually — it omits the boundary and
    // breaks parsing; the browser/axios will set it correctly for FormData.
    return axios.post(`${API_BASE_URL}/api/cameras/upload`, formData, {
